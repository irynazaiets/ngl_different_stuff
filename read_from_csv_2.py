...
with open('data/csv_files/old_1.csv', newline='') as old_file:
...
for i in data_old:
    if len(i)>=1:
        ...
        # only_numbers = re.findall(r'\b\d+\b', data)
        if len(only_numbers)>1 and only_numbers[0].isdigit():
            # if int(only_numbers[0]) in range(0, 1058, 1) and len(only_numbers)>=6:
            if int(only_numbers[0]) in range(10, 1058, 1) and len(only_numbers)>=4:
